...
function printQuote()  {
  ...
  if (newQuote['year'])  {                                             //if year exists
    html += `<span class="year"> ${newQuote['year']} </span>` 
  }
  if (newQuote['tag'])  {                                         //tagged
    html += `<span class="tag"> tagged as: ${newQuote['tag']} </span>`
  }
  html += `</p>`;
...
***/
setInterval(printQuote, 4000);   //begins the clock for 5 second wait between quotes
